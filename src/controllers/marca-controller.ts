...
class MarcaController {
  public async ld (req: Request, res: Response): Promise<Response> {
    ...
    else if (marcas && !(marcas.length > 0)) return res.status(200).json({ success: false, msg: msg.notFound('Marca') })
    else return res.status(200).json({ success: false, msg: msg.errorGet('Marca') })
  }
  ...
  public async cadastrar (req: Request, res: Response): Promise<Response> {
    ...
    if (validate) return res.status(200).json({ success: false, msg: msg.alreadyInsert('Marca') })

    const marca = await Marca.create(data)

    if (marca) return res.status(200).json({ success: true, msg: msg.successInsert('Marca') })

    return res.status(200).json({ success: false, msg: msg.errorInsert('Marca') })
  }
  ...
  public async editar (req: Request, res: Response): Promise<Response> {
    ...
    if (validate && !(`${validate._id}` === data._id)) return res.status(200).json({ success: false, msg: msg.alreadyInsert('Nome') })
    ...
    return res.status(200).json({ success: false, msg: msg.errorUpdate('Marca') })
  }
  ...
  public async deletar (req: Request, res: Response): Promise<Response> {
    ...
    if (produto) return res.status(200).json({ success: false, msg: msg.cantDelete('Marca') })
